...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _tipsNormalColor = [UIColor lightGrayColor];
        _tipsAttriColor = [UIColor redColor];
...
-(void)textViewDidChange:(UITextView *)textView
{
    ...
    if (!position) {
        ...
        if (length <= _maxLength) {
            _tipsLb.text = [NSString stringWithFormat:@"%ld/%ld",length, _maxLength];
            ...
            _tipsLb.text = [NSString stringWithFormat:@"%ld/%ld",_maxLength, _maxLength];
...
{
    if (!_placeholderLb) {
        _placeholderLb = [[UILabel alloc] initWithFrame:CGRectMake(5, 8, self.frame.size.width - 10, 20)];
...
{
    if (!_tipsLb) {
        _tipsLb = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width - 100 - 5, self.frame.size.height - _tipsFont.lineHeight - 5, 100, _tipsFont.lineHeight)];
        _tipsLb.textAlignment = NSTextAlignmentRight;
        _tipsLb.font = _tipsFont;
        _tipsLb.textColor = _tipsNormalColor;
        _tipsLb.text = [NSString stringWithFormat:@"0/%ld", _maxLength];
